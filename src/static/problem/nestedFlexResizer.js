// Resizable nested flex-boxes taken from https://stackoverflow.com/questions/28767221/flexbox-resizing
function manageResize(md, sizeProp, posProp) {
	...
	var prevGrow = 1 //Number(prev.style.flexGrow);
	var nextGrow = 1 //Number(next.style.flexGrow);
...
function setupResizerEvents() {
	document.body.addEventListener("mousedown", function (md) {
		...
		if (target.nodeType !== 1 || ! target.classList.contains("resizer")) {
			return;
		}

        var parent = target.parentNode;
		var h = parent.classList.contains("h_resize");
		var v = parent.classList.contains("v_resize");
		if (h && v) {
			return;
		} else if (h) {
